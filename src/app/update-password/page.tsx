...
export default function UpdatePasswordPage() {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (!isMounted) return;
      authSubscription = subscription; // Assign for cleanup
      ...
      if (event === 'PASSWORD_RECOVERY' && session) {
        ...
        // If still verifying and we get signed out or an initial session without a user,
        // it implies the token was invalid, expired, or already used.
        if (verificationTimeoutId) clearTimeout(verificationTimeoutId);
  ...
  useEffect(() => {
    // This effect handles redirection if link verification fails
  ...
  // If !verifyingLink AND !isRecoveryModeActive, it means verification failed.
  // The redirect useEffect should handle navigation. Show a generic message while redirecting.
  return (
    <div className="flex min-h-screen items-center justify-center bg-background">
      <Loader2 className="h-12 w-12 animate-spin text-destructive" />
      <p className="ml-4 text-muted-foreground">Processing error. Redirecting...</p>
    </div>
  );
}
